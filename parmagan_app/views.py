...
# Create your views here.
dic={}
dic["و"]=['التمثيل', 'سبب ضمني', 'إعادة الصياغة', 'الاستثناء', 'الخلفية', 'تشبيه', 'نتيجة ضمنية', 'التضاد', 'سبب مباشر', 'الشرطية', 'الربط', 'نتيجة مباشرة', 'التزامن', 'الترتيب الزمني']
...
dic["خلال"]=["تزامن"]
dic["ف"]=["نتيجة مباشرة","سبب مباشر","سبب ضمني","نتيجة ضمنية","اعادة صياغة","تمثيل","تزامن","ترتيب زمني","تضاد","خلفية"]
...
### end
        ...
        for entry in data:
            ...
            for sense in entry.get("senses", []):
                ...
                prompt = Reem_output[3].split("@")

                if "محسوس" in prompt[0]:
                    prompts = prompt[0]
                elif "فعل مضارع" not in "المعنى" and "جمع تكسير" not in "المعنى":
                    prompts = "الكلمة" + ": " + "meaning"
                else:
                    prompts = prompt[0]
                ...
                if word in dic:
                    ...
                    for i in (range(len(dic[word]))):
                        prompt = "ضع كلمة " + "'" + word + "'" + " في جملة واحدة مفيدة لا دينية بحيث تدل على العلاقة التالية : " + \
                                 dic[word][i] + "." + " وضع النتيجة في الشكل التالي : \n الكلمة: \n العلاقة:\n الجملة: "
                        print(prompt)
        ...
        # Now extracted_data contains the desired information
